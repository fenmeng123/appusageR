...
test_that("read_appusage_batch captures malformed input diagnostics", {
  ...
  expect_equal(summary$failure_family[[2]], "source_unknown_content")
  expect_match(summary$error_message[[2]], "no recognized APP Usage markers")
...
test_that("read_appusage_batch records empty recognized exports without writing RDA", {
  ...
  expect_match(summary$error_class[[1]], "appusage_header_only_source")
  expect_match(summary$error_message[[1]], "no record rows")

  metadata <- jsonlite::read_json(summary$metadata_file[[1]], simplifyVector = TRUE)
  expect_equal(metadata$processing$first_level_status, "error")
  expect_equal(metadata$processing$first_level_failure_reason, "source_header_only")
  ...
  expect_equal(metadata$source$preflight$status, "header_only")
  expect_equal(metadata$source$preflight$detected_components, "meta")
  expect_false(metadata$source$preflight$has_record_rows)
